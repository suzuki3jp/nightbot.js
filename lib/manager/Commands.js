 ...
 */
class CommandsManager extends Base_1.Base {
    _auth;
    constructor(auth) {
        super();
        this._auth = auth;
    ...
    async getCustomCommandsByMe() {
        ...
        this.isScopesEnough(index_2.APIEndPoints.getCommands.requiredScopes);
    ...
    async getCustomCommmandById(id) {
        await this._auth.refresh();
        this.isScopesEnough(index_2.APIEndPoints.getCommandById.requiredScopes);
        const res = await this.req.get({
            url: `/1/commands/${id}`,
            config: this._auth.generateReqConfig(),
        });
    ...
    async addCustomCommand(name, message, coolDown, userLevel) {
        await this._auth.refresh();
        this.isScopesEnough(index_2.APIEndPoints.addCommand.requiredScopes);
        coolDown = coolDown ?? 0;
    ...
    async editCustomCommand(id, options) {
        await this._auth.refresh();
        this.isScopesEnough(index_2.APIEndPoints.editCommand.requiredScopes);
    ...
    async deleteCustomCommand(id) {
        await this._auth.refresh();
        this.isScopesEnough(index_2.APIEndPoints.deleteCommand.requiredScopes);
    ...
    async getDefaultCommands() {
        await this._auth.refresh();
        this.isScopesEnough(index_2.APIEndPoints.getDefaultCommands.requiredScopes);
        const res = await this.req.get({
    ...
    async getDefaultCommandByName(name) {
        await this._auth.refresh();
        this.isScopesEnough(index_2.APIEndPoints.getDefaultCommandByName.requiredScopes);
        const res = await this.req.get({
    ...
    async editDefaultCommand(name, options) {
        await this._auth.refresh();
        this.isScopesEnough(index_2.APIEndPoints.editDefaultCommand.requiredScopes);
        const res = await this.req.put({
            url: `/1/commands/default/${name}`,
            body: {
                coolDown: options.coolDown,
    ...
    isScopesEnough(requiredScopes) {
        if (!(0, index_1.isIncludeRequiedScopes)(this._auth, requiredScopes)) {
